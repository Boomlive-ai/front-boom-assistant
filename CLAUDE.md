...
- **Never write a leading `/` on an internal link in `public/`.** They are relative on purpose — `style.css`, `script.js`, `api/generate`. A leading slash resolves against the host root, which on the toolbox host is the unrelated PHP app. `style.css` contains no `url()` or `@import`, so those three are the complete set. `GENERATE_API` is absolute and unaffected.
